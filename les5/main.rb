...
def create_train
  ...
  number = gets.chomp
  @trains_list.each { |train|  error if (train.name == name) && (train.number == number)}
  puts "Выберите его тип:
    1 - Пассажирский;
    2 - Грузовой."
  picked = gets.chomp.to_i
  if picked == 1
    @trains_list.push(PassengerTrain.new(name, number))
  elsif picked == 2
    @trains_list.push(CargoTrain.new(name, number))
  else
    error
  end  
end
...
def attach_van
  train_choise
  van = PassengerVan.new() if @trains_list[@train_number - 1].type == "Пассажирский"
  van = CargoVan.new() if @trains_list[@train_number - 1].type == "Грузовой"
...
def train_choise
  if !@trains_list.empty?
    ...
    @trains_list.each { |value| puts "#{@trains_list.index(value) + 1}) #{value.name}"}
